...
async def run_client(
    ...
    if inject_proxy and proxy_mode == "forward":
        ...
        if client in {"dsh", "pi"}:
            ...
            # when built-in environment proxy support is enabled. DeepSeek Harness
            # uses the same Node fetch behavior when forward mode is requested.
...
def _detect_dsh_target() -> str:
    """Auto-detect the DeepSeek Harness API endpoint."""
